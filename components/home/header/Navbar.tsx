...
const Navbar = () => {
    ...
    return (
        <div className='sticky top-0 z-50 w-full  boder-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60'>
            <div className='container mx-auto px-4 sm:px-6 lg:px-8'>
                <div className='flex items-center justify-between h-16'>
                    ...
                    <Button variant={'ghost'} size={'icon'} className='md:hidden text-muted-foreground hover:text-foreground' onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
                        {
                            isMobileMenuOpen ? (<X className="h-5 w-5" />) : (<Menu className='h-3' />)
                        }
                    </Button>
                </div>
            </div>
